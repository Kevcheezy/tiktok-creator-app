...
The engineering roadmap lives in `ENGINEERING_ROADMAP.md` — a long markdown file that's hard to scan for current status. There's no way to see at a glance what's being worked on, by whom, or what's blocked. The admin must read through hundreds of lines to understand project state.

## Solution

A `/roadmap` page that parses `ENGINEERING_ROADMAP.md` at render time into a live Kanban board with three columns (Backlog / In Progress / Done). Each task card shows its assigned worker — an FF7 character that represents a specific engineering domain. The markdown file remains the single source of truth.
...
|----------|--------|-----------|
| Data source | Parse ENGINEERING_ROADMAP.md at render time | Markdown is the single source of truth, no sync drift |
...
```
ENGINEERING_ROADMAP.md (git-tracked, single source of truth)
        ↓
GET /api/roadmap
  → Reads file from filesystem (server-side)
  → Parses markdown into structured task objects
  → Joins with roadmap_worker table for assignments
  → Returns JSON array of tasks with status, tier, worker, progress
        ↓
/roadmap page (Kanban board)
  → Renders 3 columns: Backlog | In Progress | Done
  → Task cards with FF7 worker avatars
...
| `#### R1.5.1 - Title 🔧 IN PROGRESS` | `in_progress` | Wrench emoji + IN PROGRESS |
...
interface RoadmapTask {
  ...
  dependsOn: string[];     // ["R1.1", "B0.8"] — parsed from "Depends on:" line
  ...
  section: string;         // "Data model", "API", "Frontend", "Pipeline" — for auto-assignment
}
```
...
function parseRoadmap(markdown: string): RoadmapTask[] {
  ...
  //   2. Detect status from ~~strikethrough~~ DONE/FIXED or 🔧 IN PROGRESS
...
```
┌─────────────────────────────────────────────────────────────────────────┐
│  ENGINEERING ROADMAP                                    [Filter ▾] 🔍  │
│  ┌──────────┬──────────┬──────────┬──────────┬──────────┐              │
│  │ ☁️ Cloud  │ 👊 Tifa  │ 💪 Barret│ 🌸 Aerith│ 🔥 Red   │  [All]     │
│  │ 3 tasks  │ 2 tasks  │ 0 tasks  │ 1 task   │ 0 tasks  │              │
│  └──────────┴──────────┴──────────┴──────────┴──────────┘              │
├─────────────────────┬─────────────────────┬─────────────────────────────┤
│     BACKLOG (5)     │  IN PROGRESS (2)    │        DONE (14)           │
│                     │                     │                             │
│  ┌─────────────────┐│ ┌─────────────────┐ │ ┌─────────────────┐        │
│  │ ☁️ Cloud   Tier 1││ │ 👊 Tifa   Tier 1.5│ │ ☁️ Cloud   Tier 1│        │
│  │                 ││ │                 │ │ │                 │        │
│  │ R1.7 B-Roll     ││ │ R1.5.1 Inflcr  │ │ │ ~~R1.1~~ Asset  │        │
│  │ Agent           ││ │ Edit UI        │ │ │ Generation      │        │
│  │                 ││ │                 │ │ │                 │        │
│  │ ████████░░ 0%   ││ │ ██████░░░░ 50% │ │ │ ██████████ 100% │        │
│  │ Depends: R1.1   ││ │                 │ │ │                 │        │
│  └─────────────────┘│ └─────────────────┘ │ └─────────────────┘        │
│                     │                     │                             │
│  ┌─────────────────┐│ ┌─────────────────┐ │ ┌─────────────────┐        │
│  │ ☁️ Cloud   Tier 2││ │ 🌸 Aerith Tier 2│ │ 💪 Barret  Tier 1│        │
│  │                 ││ │                 │ │ │                 │        │
│  │ R2.0 KPI        ││ │ R2.5 Ref Video │ │ │ ~~R1.4~~ Obs.   │        │
│  │ Dashboard       ││ │ Intel Spec     │ │ │ & Logging       │        │
│  │                 ││ │                 │ │ │                 │        │
│  │ ░░░░░░░░░░ 0%   ││ │ ████░░░░░░ 30% │ │ │ ██████████ 100% │        │
│  │ Depends: R1.2   ││ │                 │ │ │                 │        │
│  └─────────────────┘│ └─────────────────┘ │ └─────────────────┘        │
│                     │                     │                             │
│  ...more cards...   │                     │ ...more cards (scrollable)..│
└─────────────────────┴─────────────────────┴─────────────────────────────┘
```
...
```
┌──────────────────────────────────┐
│ ☁️ Cloud Strife          Tier 1  │  ← Worker avatar + name, tier badge
│                                  │
│ R1.7 B-Roll Agent               │  ← Task ID + title (bold)
│ Two-phase B-roll planning and    │  ← Description (truncated, 1 line)
│ generation with storyboard...   │
│                                  │
│ ████████░░ 60%  (12/20)         │  ← Progress bar from checkboxes
│                                  │
│ 📎 Spec  ⛓️ R1.1               │  ← Spec link, dependency badges
│ 💰 ~$1.13/video                 │  ← Cost impact if present
└──────────────────────────────────┘
```
...
|--------|-------|-------------|
| `GET` | `/api/roadmap` | Parse ENGINEERING_ROADMAP.md → JSON array of `RoadmapTask` objects |
...
`/roadmap` — new page at `src/app/roadmap/page.tsx`
...
- [ ] Auto-assignment heuristic: domain keywords → FF7 character mapping
...
### Data Integrity
- [ ] Board always reflects current state of ENGINEERING_ROADMAP.md (no stale data)
...
Recommendation:
  Step 1: Task A + Task B in parallel (parser + worker system — independent)
